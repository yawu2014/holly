...
public class Decoder {
    ...
    static {
        file = new File("C:\\Users\\JK\\PhpstormProjects\\decoder\\httprequest_original_.php");
        if (file.exists()) {
            file.delete();
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        List<String> lines = FileUtil.readLines(new File("C:\\Users\\JK\\PhpstormProjects\\decoder\\httprequest_original.php"), Charset.defaultCharset());
        for (String line : lines) {
            FileUtil.appendLines(Lists.newArrayList(line), file, Charset.defaultCharset());
        ...
        FileUtil.appendLines(Lists.newArrayList(blankNumber(1) + " {"), file, Charset.defaultCharset());

        startWrite(7, false, lines);

        FileUtil.appendLines(Lists.newArrayList(blankNumber(1) + " }"), file, Charset.defaultCharset());
        FileUtil.appendLines(Lists.newArrayList("} ?>"), file, Charset.defaultCharset());
    }
            ...
            if (isIfBlock(lineInfos)) {
                ...
                for (LineInfo newLineInfo : newLineInfos) {
                    if (!isGotoFlag(newLineInfo.getContent())) {
                        ...
                        FileUtil.appendLines(Lists.newArrayList(newLineInfo.getContent()), file, Charset.defaultCharset());
                    } else {
    ...
    private static void dealIfBlock(List<LineInfo> lineInfos, List<String> lines) throws Exception {
        ...
        if (isIfDeCondition(content)) {
            ...
            //写该if语句
            FileUtil.appendLines(Lists.newArrayList(originalCondition), file, Charset.defaultCharset());
            //获得即将执行的下一行语句
            ...
            //写该if语句
            FileUtil.appendLines(Lists.newArrayList(content), file, Charset.defaultCharset());
            //获得即将执行的下一行语句
            startWrite(lineInfos.get(1).getLine(), false, lines);

            //执行完了之后，拼接else
            FileUtil.appendLines(Lists.newArrayList(blankNumber(4) + "}else{"), file, Charset.defaultCharset());
            startWrite(lineInfos.get(3).getLine(), false, lines);
            //执行完了之后在最后添加}
            FileUtil.appendLines(Lists.newArrayList(blankNumber(4) + "}"), file, Charset.defaultCharset());
        }
        if (goOnLineIndex != -1) {
